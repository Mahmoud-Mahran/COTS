...
void DIO_voidSetPinValue(u8 port, u8 pin, u8 Value) {
	...
	if (port < 3 && pin < 16) {
		if (Value == GPIO_HIGH) {
			...
			switch (port) {
			case PORTA:
				SET_BIT(GPIOA_BRR, pin);
				break;
			case PORTB:
				SET_BIT(GPIOB_BRR, pin);
				break;
			case PORTC:
				SET_BIT(GPIOC_BRR, pin);
				break;
			}
		}
...
u8 DIO_u8GetPinValue(u8 port, u8 pin) {
	...
	if (port < 3 && pin < 16) {
		...
		switch (port) {
		case PORTA:
			Local_u8Val = GET_BIT(GPIOA_IDR, pin);
			break;
		case PORTB:
			Local_u8Val = GET_BIT(GPIOB_IDR, pin);
			break;
		case PORTC:
			Local_u8Val = GET_BIT(GPIOC_IDR, pin);
			break;
		}
	} else {/*	Return Error	*/
	}
